...
public class KodiHandler extends BaseThingHandler {
    ...
    private void showNotification(Command command) {
        if (!(command instanceof StringType)) {
            logger.error("The Kodi Item: {} supports only string commands", CHANNEL_GUI_SHOW_NOTIFICATION_CHANNEL);
            return;
        }
    ...
    private void playFile(Command command) {
        if (!(command instanceof StringType)) {
            logger.error("The Kodi Item: {} supports only string commands", CHANNEL_PLAY_FILE);
            return;
        }
    ...
    private void playerStop(Command command) {
        if (!(command instanceof OnOffType)) {
            logger.error("The Kodi Item: {} supports only OnOff commands", CHANNEL_STOP);
            return;
        }
    ...
    private boolean startClient() {
        try {
            ...
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.COMMUNICATION_ERROR, "Can not connect to Kodi");
            e.printStackTrace();
            return false;
        }
        return true;
